...
pub async fn contest_state_update(
    ...
    if let Some(current_state) =
        queries::contest_states::get_contest_state(db_pool, &contest_state.contest_id).await?
    {
...
fn validate_contest_state_transition(
...
mod tests {
    ...
    fn test_valid_contest_state_transitions() {
        ...
        assert!(validate_contest_state_transition(
            &ContestStatus::Setup,
        ...
        assert!(
            validate_contest_state_transition(&ContestStatus::Setup, &ContestStatus::Setup).is_ok()
        );
        assert!(validate_contest_state_transition(
            &ContestStatus::Registration,
            &ContestStatus::Registration
        ...
        .is_ok());
        assert!(
            validate_contest_state_transition(&ContestStatus::Paused, &ContestStatus::Paused)
                .is_ok()
        );
        assert!(validate_contest_state_transition(
            &ContestStatus::Complete,
    ...
    fn test_invalid_contest_state_transitions() {
        ...
        .is_err());
        assert!(
            validate_contest_state_transition(&ContestStatus::Complete, &ContestStatus::Setup)
                .is_err()
        );
        ...
        .is_err());
        assert!(
            validate_contest_state_transition(&ContestStatus::Setup, &ContestStatus::Complete)
                .is_err()
        );
        assert!(validate_contest_state_transition(
            &ContestStatus::Registration,
