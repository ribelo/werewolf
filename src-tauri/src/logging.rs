...
pub fn init_tracing() -> Result<(PathBuf, WorkerGuard), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "BACKEND: Logging system initialized with daily rotation, log file: {}, level: {}",
        log_path.display(),
        current_log_level
    );
    tracing::info!(
        "Logging system initialized with daily rotation, log file: {}, level: {}",
        log_path.display(),
        current_log_level
    );
...
pub async fn write_log(
    ...
    write_frontend_log(&level, &message, timestamp.as_deref())
        .await
